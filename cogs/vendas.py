...
def calculate_robux_price(amount: int) -> float:
    ...
    # Usa preços fixos para valores comuns para dar um "desconto"
...
class Vendas(commands.Cog):
    ...
    async def calculadora(self, interaction: discord.Interaction, robux: int):
        ...
        embed = discord.Embed(
            title="🧮 Calculadora de Game Pass",
            description=f"Para o comprador receber **{robux} Robux**, você precisa criar uma Game Pass no valor de **{gamepass_value} Robux**.",
            color=ROSE_COLOR
        )
        await interaction.response.send_message(embed=embed)
            ...
            async def callback(self, select_interaction: discord.Interaction):
                ...
                if selected_product == "Robux":
                    ...
                    for item, price in product_info["prices"].items():
                        final_price = price
                        if is_vip and product_info.get("vip_discount", {}).get(item):
