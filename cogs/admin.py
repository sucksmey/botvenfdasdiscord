...
from cogs.cliente import CustomerAreaView, ReviewView

desconto_group = app_commands.Group(name="desconto", description="Gerenciamento de desconto global.", guild_ids=[config.GUILD_ID])
...
class Admin(commands.Cog):
    ...
    @desconto_group.command(name="aplicar", description="Aplica um desconto global em todas as novas compras.")
    @app_commands.checks.has_role(config.ADMIN_ROLE_ID)
    @app_commands.describe(porcentagem="A porcentagem de desconto (ex: 10 para 10%).")
    async def aplicar_desconto(self, interaction: discord.Interaction, porcentagem: int):
        if not (0 < porcentagem <= 100):
            await interaction.response.send_message("A porcentagem de desconto deve ser entre 1 e 100.", ephemeral=True)
            return
        
        config.CURRENT_DISCOUNT = porcentagem
        await interaction.response.send_message(f"✅ Desconto global de **{porcentagem}%** ativado para todos os novos tickets!", ephemeral=True)
    ...
    async def remover_desconto(self, interaction: discord.Interaction):
        if config.CURRENT_DISCOUNT == 0:
            await interaction.response.send_message("Não há nenhum desconto ativo para remover.", ephemeral=True)
            return
            
        config.CURRENT_DISCOUNT = 0
        await interaction.response.send_message("✅ Desconto global removido! Os preços voltaram ao normal.", ephemeral=True)
    ...
    async def cleanup_loop(self):
        ...
        try:
            async with database.engine.connect() as conn:
                ...
                transcript_channel = self.bot.get_channel(config.TRANSCRIPT_CHANNEL_ID)
    ...
    async def pix(self, interaction: discord.Interaction):
        pix_embed = discord.Embed(title="Pagamento via PIX", description="Use o QR Code acima ou a chave PIX (E-mail) enviada abaixo.", color=config.ROSE_COLOR)
        pix_embed.set_footer(text="Após pagar, por favor, envie o comprovante neste chat.")
        pix_embed.set_image(url=config.QR_CODE_URL)
    ...
    async def atender(self, interaction: discord.Interaction):
        ...
        async def perform_attend_logic(is_memory_ticket=True):
            try:
                ...
                if channel.id in config.ONGOING_SALES_DATA:
                    config.ONGOING_SALES_DATA[channel.id]['handler_admin_id'] = admin_user.id
            except Exception as e:
        ...
        if channel.topic and "ID: " in channel.topic:
            await interaction.response.send_message("Recuperando ticket da memória...", ephemeral=True)
            try:
                client_id = int(channel.topic.split("ID: ")[1].strip())
                config.ONGOING_SALES_DATA[channel.id] = {'client_id': client_id, 'status': 're-attended'}
    ...
    @app_commands.command(name="aprovar", description="[Admin] Aprova a compra e arquiva o ticket.")
    @app_commands.guilds(discord.Object(id=config.GUILD_ID))
    @app_commands.checks.has_role(config.ADMIN_ROLE_ID)
    @app_commands.describe(produto="[Opcional] Nome do produto, se o bot esqueceu.", valor="[Opcional] Valor da compra. Ex: 4.50")
    async def aprovar(self, interaction: discord.Interaction, produto: str = None, valor: str = None):
        await interaction.response.defer()
        channel = interaction.channel
        ticket_data = config.ONGOING_SALES_DATA.get(channel.id)
        if not ticket_data:
            if channel.topic and "ID: " in channel.topic:
                try:
                    client_id = int(channel.topic.split("ID: ")[1].strip())
                    if produto and valor is not None:
                     ...
                     await interaction.followup.send("❌ Não foi possível recuperar os dados do cliente.", ephemeral=True); return
            else:
        ...
        try:
            async with database.engine.connect() as conn:
                result = await conn.execute(database.transactions.insert().values(user_id=membro.id, user_name=membro.name, channel_id=channel.id, product_name=final_product_name, price=final_price, gamepass_link=ticket_data.get("gamepass_link"), handler_admin_id=interaction.user.id, delivery_admin_id=config.ROBUX_DELIVERY_USER_ID, timestamp=datetime.utcnow(), closed_at=datetime.utcnow()).returning(database.transactions.c.id))
                new_transaction_id = result.scalar_one()
                await conn.commit()
            logging.info(f"Transação ID {new_transaction_id} salva no DB.")
        except Exception as e:
            logging.error(f"Falha ao salvar a transação no DB: {e}"); await interaction.followup.send("⚠️ Erro ao salvar a transação.", ephemeral=True); return

        log_channel = self.bot.get_channel(config.LOGS_COMPRAS_CHANNEL_ID)
        if log_channel:
            log_embed = discord.Embed(title="✅ Log de Compra", color=discord.Color.green(), timestamp=datetime.now(config.BR_TIMEZONE))
        ...
        try:
            dm_embed = discord.Embed(title="❤️ Obrigado pela sua compra!", description=f"Sua compra de **{final_product_name}** foi concluída.", color=config.ROSE_COLOR)
            dm_embed.set_thumbnail(url=config.IMAGE_URL_FOR_EMBEDS)
        ...
        entregues_category = interaction.guild.get_channel(config.CATEGORY_ENTREGUES_ID)
        ...
        if channel.id in config.ONGOING_SALES_DATA:
            del config.ONGOING_SALES_DATA[channel.id]
    ...
    async def aprovar_vip(self, interaction: discord.Interaction, membro: discord.Member):
        ...
        if not vip_role: await interaction.followup.send("Erro: O cargo VIP não foi encontrado.", ephemeral=True); return
        try:
            await membro.add_roles(vip_role, reason=f"Compra de VIP aprovada por {interaction.user.name}")
            async with database.engine.connect() as conn:
                await conn.execute(database.transactions.insert().values(user_id=membro.id, user_name=membro.name, channel_id=interaction.channel.id, product_name="Assinatura VIP", price=config.VIP_PRICE, handler_admin_id=interaction.user.id, timestamp=datetime.utcnow(), closed_at=datetime.utcnow()))
                await conn.commit()
            ...
            await interaction.followup.send("Assinatura VIP aprovada!", ephemeral=True)
        except Exception as e:
...
async def setup(bot: commands.Bot):
    bot.tree.add_command(desconto_group, guild=discord.Object(id=config.GUILD_ID))
